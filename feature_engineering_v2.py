...
用途：
  整合 feature_test_1.py 與 modified_feature_engineering.py 的所有特徵，
  並修正已知 bug、補強鏈別風險特徵、修正 IsolationForest 套用方式。

主要改進（相較於 feature_test_1.py）：
  1. 修正所有 ("id", "count") bug：
       twd_transfer / usdt_twd_trading / usdt_swap 均無 id 欄位，
       改用各表必定存在的欄位（kind / is_buy / kind）做 count。
  2. 整合 modified_feature_engineering.py 的鏈別風險特徵：
       TRC20/BSC 比例、夜間比例、地址重用率、流向不平衡、協議切換率。
  3. 新增 build_protocol_detail()：per-user 協議細粒度特徵。
  4. 修正 IsolationForest：
       以 train+test 合併後的特徵矩陣 fit，再分別 transform，
       避免 train/test 分布不一致。
  5. 新增交叉特徵：trc20_night_cross、bsc_addr_ip_cross、
       protocol_switch_fast_cross 等。
  6. 移除所有 IPython display() / plot_*() 呼叫，純 .py 腳本可直接執行。

輸出：
  - train_feature_v2.csv
  - test_feature_v2.csv
  - feature_full_v2.csv

注意：
  - 金額縮放：ori_samount / twd_srate / trade_samount / twd_samount /
    currency_samount 均需 * 1e-8。
  - Protocol enum：0=Self, 1=ERC20, 2=OMNI, 3=BNB, 4=TRC20, 5=BSC, 6=Polygon
"""
...
def build_feature_dataset() -> tuple:
    ...
    feature_df.to_csv("feature_full_v2.csv",     index=False)
